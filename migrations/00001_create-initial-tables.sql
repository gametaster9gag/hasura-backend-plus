...
SELECT create_constraint_if_not_exists('public.platform_users', 'users_pkey', 'PRIMARY KEY (id);');
...
SELECT create_constraint_if_not_exists('auth.accounts', 'accounts_user_id_fkey', 'FOREIGN KEY (user_id) REFERENCES public.platform_users(id) ON UPDATE CASCADE ON DELETE CASCADE;');
...
DROP TRIGGER IF EXISTS set_public_users_updated_at ON public.platform_users;
CREATE TRIGGER set_public_users_updated_at BEFORE UPDATE ON public.platform_users FOR EACH ROW EXECUTE FUNCTION public.set_current_timestamp_updated_at();
